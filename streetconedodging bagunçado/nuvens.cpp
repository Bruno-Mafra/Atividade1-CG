...
void Nuvens::initializeGL(GLuint program, int quantity) {
  ...
  bool i;
   ...
   do { // gera nuvens aleatoriamente longe uma das outras
     aux = nuvem.m_translation; 
      nuvem.m_translation = {m_randomDist(m_randomEngine), m_randomDist(m_randomEngine)}; 
      for (auto &nuvem : m_nuvens) {
        if(aux[0] - nuvem.m_translation[0] > 1.0f && aux[1] - nuvem.m_translation[1] > 1.0f ) {
          i = true;
        } else {
            i = false;
             }
